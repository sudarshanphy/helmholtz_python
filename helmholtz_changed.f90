...
! modified gamma + radiation pressure 
      subroutine modgamma(temp, dens, abar, zbar, pres, eint, entr)
          ...
          double precision :: pres_ideal, pres_rad, eint_ideal, eint_rad, &
                              entr_ideal, entr_rad
          ...
          pres_rad = (1.0d0/3.0d0) * asol *&
                      temp * temp * temp * temp 
          eint_ideal = (3.0d0/2.0d0) * eos_gas * temp / abar
          eint_rad = asol * temp *temp *temp *temp / dens 
          
          x = abar * abar * sqrt(abar) / (dens * avo)
          s = 2.0d0 * pi * amu * kerg * temp / (h*h)
          z = x * s* sqrt(s)
...
07    format(1x,t2,a7,1p7e16.8) !sneo
          !write (6, *) "----------------------------------------------------"
          !write (6, *) "modified gamma: "
          !write (6, 07) "pres = ", pres 
          !write (6, 07) "eint = ", eint
          !write (6, 07) "entr = ", entr
...
! routine helmeos computes the pressure, energy and entropy via tables
      ...
      include 'implno.dek'
      include 'helm_table_storage_changed.dek'
...
!      write(6,*)
      ...
      subroutine helmeos(temp, den, abar, zbar, pres, eint, entr)
      include 'implno.dek'
      include 'const.dek'
...
! declare
      double precision, intent(in) :: den, temp, abar, zbar
      double precision, intent(out) :: pres, eint, entr
      integer          i,j
      double precision ytot1,ye, &
                       ...
                       sion,xnem,pele,eele,sele,ener,dpresdd, &
...
! electron chemical potential, positron chemical potential and its derivatives
       ...
       write (6, *) "----------------------------------------------------"
       write (6, *) "helmholtz: "
       write (6, 07) "pres = ", ptot_row(j)
       write (6, 07) "eint = ", etot_row(j)
       write (6, 07) "entr = ", stot_row(j) 
       call modgamma(temp_row(j), den_row(j), abar_row(j), zbar_row(j), &
                     ptot_row(j), etot_row(j), stot_row(j))
       write (6, *) "----------------------------------------------------"
       write (6, *) "modified gamma: "
       write (6, 07) "pres = ", ptot_row(j)
       write (6, 07) "eint = ", etot_row(j)
