...
! routine etages makes a good guess for the electron chemical potential
      ...
      subroutine eosfxt(temp, den, abar, zbar, pres, eint, entr)
      include 'implno.dek'
      include 'const.dek'
...
! declare the input
      ...
      double precision, intent(out) :: pres, eint, entr

! declare local variables
! totals
      double precision dpresdd,dpresdt,dpresda,dpresdz, &
...
! start pipeline loop
      do j=jlo_eos,jhi_eos
       ...
       if ((temp .le. 0.0) .or. (den .le. 0.0)) then
...
! declare the pass
      integer, intent(in)  ::         n
      double precision, intent(in) :: x,par(n)
...
! declare
      ...
      integer          :: j
      double precision, intent(in) :: a,b, par(n)
      double precision, intent(out) :: result,drdeta,drdtheta, &
                       drdeta2,drdtheta2,drdetadtheta 
      double precision :: absc1,absc2,center,hlfrun,wg(10),xg(10), &
...
!           result - result of the 20-point gauss formula
      ...
      do j=1,10
        ...
        if (f == 1) then
        call fdfunc1(absc1, par, n, fval(1), deta(1), dtheta(1), &
               deta2(1), dtheta2(1), detadtheta(1))
        call fdfunc1(absc2, par, n, fval(2), deta(2), dtheta(2), &
               deta2(2), dtheta2(2), detadtheta(2))
        else if (f == 2) then
        call fdfunc2(absc1, par, n, fval(1), deta(1), dtheta(1), &
               deta2(1), dtheta2(1), detadtheta(1))
        call fdfunc2(absc2, par, n, fval(2), deta(2), dtheta(2), &
               deta2(2), dtheta2(2), detadtheta(2))
        end if
...
! declare
      ...
      double precision, intent(out) :: result,drdeta,drdtheta, &
                       drdeta2,drdtheta2,drdetadtheta 
      double precision :: absc,wg(20),xg(20),fval,deta,dtheta, &
...
!           result - result of the 20-point gauss formula
      ...
      do j=1,20
       absc = a+b*xg(j)
       if (f == 1) then
       call fdfunc1(absc, par, n, fval, deta, dtheta, &
              deta2,dtheta2,detadtheta)
       else if (f == 2) then
       call fdfunc2(absc, par, n, fval, deta, dtheta, &
              deta2,dtheta2,detadtheta)
       end if
...
! declare
      ...
      !integer  fdfunc1=1,fdfunc2=2
      double precision, intent(in) ::  dk,eta,theta
      double precision, intent(out) :: fd,fdeta,fdtheta, &
                       fdeta2,fdtheta2,fdetadtheta
...
! 14 significant figure accuracy
      call dqleg020(2, 0.d0,  s12, res(1), drde(1), drdt(1), &
                    drde2(1), drdt2(1), drdet(1), par,3)
      call dqleg020(1,   s1,   s2, res(2), drde(2), drdt(2), &
                    drde2(2), drdt2(2), drdet(2), par,3)
      call dqleg020(1,   s2,   s3, res(3), drde(3), drdt(3), &
                    drde2(3), drdt2(3), drdet(3), par,3)
      call dqlag020(1,   s3, 1.d0, res(4), drde(4), drdt(4), &
